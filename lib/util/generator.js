...
const filedLayoutBuilder = (antdForm, config, defaultValue = {}) => {
  ...
  const cols = Array.isArray(fields) && fields.map((field) => {
    ...
    return (
      <Col key={`smart_${field.key}`} span={(24 / columnCount) * (field.span || 1)}>
        <FormItem
          key={field.key}
          label={field.label}
        >
...
const formBuilder = (antdForm, config, defaultValue = {}) => (
  <Form>
    <Row gutter={32}>
      {
          filedLayoutBuilder(antdForm, config, defaultValue)
      }
    </Row>
  </Form>
);

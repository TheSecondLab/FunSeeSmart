...
const smart = (config = {}) => (WrappedComponent) => {
  ...
  class MagicComponent extends C {
    ...
    componentDidMount() {
      ...
      // dynamic get all schema select options
      const targetFileds = [...querySchema.fields, ...editSchema.fields];
      const getAllSelectFields = (fieldArr,result = []) => {
        if(Array.isArray(fieldArr)){
          fieldArr.forEach(field => {
            if(field.type === FIELD_TYPE.SELECT && field.url && !field.done && typeof field.formatter === 'function'){
              result.push(field);
            } else if(field.type === FIELD_TYPE.GROUP && Array.isArray(field.fields)){
              getAllSelectFields(field.fields, result);
            }
          })
        }
      }
      const selectFields = [];
      getAllSelectFields(targetFileds, selectFields);
      selectFields.forEach(item => {
      request[item.method || 'post'](item.url, item.param || {}).then((data) => {
        item.options = item.formatter(data);
      });
        item.done = true;
      });
    }
  ...
  return connect(
    ...
    , dispatch => ({
      ...
      smartClearQueryParam: bindActionCreators(actions.clearQueryParam, dispatch)
    })
  )(MagicComponent);
};
